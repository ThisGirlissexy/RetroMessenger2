...
RetroMessengerMessageSentDlg::RetroMessengerMessageSentDlg( wxWindow* parent )
:
MessageSentDlg( parent, -1 )
{
	// Resize to fit the contents.
		this->GetSizer()->SetSizeHints( this );
	  // About dialog's event loop.
// Not needed because we only have a default OK button.
//BEGIN_EVENT_TABLE( RetroMessengerMessageSentDlg, MessageSentDlg )
	//EVT_BUTTON( wxID_OK, RetroMessengerMessageSentDlg::OnOK );
//END_EVENT_TABLE()
